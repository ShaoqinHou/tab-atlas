...
import { applyReadyProposals } from '../app/organization-actions.js';
import { buildWorkingSet } from '../app/working-set.js';
...
async function apiRoute(req,res,url,{store,bridge,jobs,runtime,evidence}){
  const m=req.method;
  if(m==='GET'&&url.pathname==='/api/dashboard')return send(res,200,{stats:store.stats(),latestCapture:store.latestCapture(),collections:store.collectionsTree(),jobs:store.listJobs(8),pairings:bridge.listPairings(),history:store.recentHistory(6)});
  if(m==='GET'&&url.pathname==='/api/resources')return send(res,200,store.listResources({q:url.searchParams.get('q')||'',collectionId:url.searchParams.get('collection')||'',state:url.searchParams.get('state')||'',type:url.searchParams.get('type')||'',view:url.searchParams.get('view')||'active',limit:Math.min(100,Number(url.searchParams.get('limit')||60)),offset:Number(url.searchParams.get('offset')||0)}));
  ...
  if(m==='POST'&&url.pathname==='/api/working-set'){const b=await body(req);return send(res,200,await buildWorkingSet({store,runtime,query:b.query,useAgent:!!b.useAgent,model:b.model,reasoningEffort:b.reasoningEffort}));}
  const undo=match(url.pathname,/^\/api\/history\/([^/]+)\/undo$/);if(m==='POST'&&undo)return send(res,200,store.undo(undo));
  if(m==='POST'&&url.pathname==='/api/organize'){const b=await body(req);let ids=b.resourceIds||[];if(b.scope==='all'||b.scope==='filtered')ids=store.listResources({q:b.q||'',collectionId:b.collectionId||'',state:b.state||'',type:b.type||'',view:b.view||'active',limit:100000}).items.map(x=>x.id);const id=store.createJob({kind:b.wholeLibrary?'reorganize':'organize',resourceIds:ids,scope:{mode:b.scope||'selected',delegated:!!b.delegated,contractVersion:'organization-v1'}});queueMicrotask(async()=>{try{await jobs.runOrganization(id,{useAgent:!!b.useAgent,model:b.model,reasoningEffort:b.reasoningEffort});if(b.delegated)applyReadyProposals(store,ids,{delegated:true});}catch{}});return send(res,202,{jobId:id,targetCount:ids.length});}
  ...
  if(m==='POST'&&url.pathname==='/api/proposals/apply'){const b=await body(req);return send(res,200,applyReadyProposals(store,b.resourceIds||[],{delegated:!!b.delegated}));}
  if(m==='POST'&&url.pathname==='/api/proposals/reject'){const b=await body(req);for(const id of b.proposalIds||[])store.decideProposal(id,'rejected');return send(res,200,{rejected:(b.proposalIds||[]).length});}
  if(m==='POST'&&url.pathname==='/api/memberships/remove'){const b=await body(req);for(const id of b.resourceIds||[])store.removeMembership(id,b.collectionId,{actor:'user'});return send(res,200,{ok:true});}
  if(m==='POST'&&url.pathname==='/api/resources/archive'){const b=await body(req);for(const id of b.resourceIds||[])store.archiveResource(id,b.archived!==false,{actor:'user'});return send(res,200,{ok:true});}
  if(m==='POST'&&url.pathname==='/api/resources/delete'){const b=await body(req);for(const id of b.resourceIds||[])store.deleteResource(id,true,{actor:'user'});return send(res,200,{ok:true});}
