...
const store=new Store(path.join(state,'tabatlas.db'));store.recoverInterruptedJobs();const agentCwd=path.join(state,'agent-context');fs.mkdirSync(agentCwd,{recursive:true});const runtime=new CodexAppServer({defaultCwd:agentCwd});const bridge=new BrowserBridge({store});const jobs=new JobRunner({store,runtime});const evidence=new PublicEvidenceService();
