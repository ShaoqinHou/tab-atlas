...
export class Store {
  ...
  listResources({q='',collectionId='',state='',type='',view='active',limit=60,offset=0}={}){
    const where=['r.deleted_at IS NULL'], args=[];
    if(view==='active')where.push('r.archived_at IS NULL');
    else if(view==='archived')where.push('r.archived_at IS NOT NULL');
    if(q){where.push('(r.title LIKE ? OR r.canonical_url LIKE ? OR EXISTS(SELECT 1 FROM notes n WHERE n.resource_id=r.id AND n.text LIKE ?) OR EXISTS(SELECT 1 FROM occurrences o WHERE o.resource_id=r.id AND o.group_title LIKE ?))'); const s=`%${q}%`;args.push(s,s,s,s);}
    ...
    const items=this.db.prepare(`SELECT r.*,
      ...
      (SELECT payload_json FROM evidence ev WHERE ev.resource_id=r.id ORDER BY ev.acquired_at DESC LIMIT 1) evidence_payload,
      (SELECT limitation FROM evidence ev WHERE ev.resource_id=r.id ORDER BY ev.acquired_at DESC LIMIT 1) evidence_limitation
      FROM resources r WHERE ${ws} ORDER BY r.updated_at DESC LIMIT ? OFFSET ?`).all(...args,limit,offset);
    return {total:Number(total),limit,offset,items:items.map(x=>({...x,occurrence_count:Number(x.occurrence_count),proposal_count:Number(x.proposal_count),preview:p(x.evidence_payload,null)}))};
  }
  getResource(id){
  ...
  removeMembership(resourceId,collectionId,{actor='user',record=true}={}){ const old=this.db.prepare('SELECT * FROM memberships WHERE resource_id=? AND collection_id=?').get(resourceId,collectionId);if(!old)return;this.db.prepare('DELETE FROM memberships WHERE resource_id=? AND collection_id=?').run(resourceId,collectionId);this.refreshOrganizationState(resourceId);if(record)this.recordHistory('membership.remove',{resourceId,collectionId},{type:'membership.add',resourceId,collectionId,origin:old.origin,pinned:!!old.pinned},actor); }
  ...
  undo(id){const h=this.db.prepare('SELECT * FROM history WHERE id=?').get(id);if(!h)throw new Error('History not found');if(h.undone_at)return {alreadyUndone:true};const inv=p(h.inverse_json);if(inv.type==='membership.remove')this.removeMembership(inv.resourceId,inv.collectionId,{record:false});else if(inv.type==='membership.add')this.setMembership(inv.resourceId,inv.collectionId,{origin:inv.origin,pinned:inv.pinned,record:false});else if(inv.type==='resource.unarchive')this.archiveResource(inv.resourceId,false,{record:false});else if(inv.type==='resource.archive')this.archiveResource(inv.resourceId,true,{record:false});else if(inv.type==='resource.restore')this.deleteResource(inv.resourceId,false,{record:false});else if(inv.type==='resource.delete')this.deleteResource(inv.resourceId,true,{record:false});else throw new Error('Unsupported undo');this.db.prepare('UPDATE history SET undone_at=? WHERE id=?').run(now(),id);return {undone:true};}
  archiveResource(resourceId,archived=true,{actor='user',record=true}={}){const r=this.db.prepare('SELECT archived_at FROM resources WHERE id=? AND deleted_at IS NULL').get(resourceId);if(!r)throw new Error('Resource not found');const was=!!r.archived_at;if(was===archived)return {changed:false};this.db.prepare('UPDATE resources SET archived_at=?,updated_at=? WHERE id=?').run(archived?now():null,now(),resourceId);if(record)this.recordHistory(archived?'resource.archive':'resource.unarchive',{resourceId},{type:archived?'resource.unarchive':'resource.archive',resourceId},actor);return {changed:true};}
  deleteResource(resourceId,deleted=true,{actor='user',record=true}={}){const r=this.db.prepare('SELECT deleted_at FROM resources WHERE id=?').get(resourceId);if(!r)throw new Error('Resource not found');const was=!!r.deleted_at;if(was===deleted)return {changed:false};this.db.prepare('UPDATE resources SET deleted_at=?,updated_at=? WHERE id=?').run(deleted?now():null,now(),resourceId);if(record)this.recordHistory(deleted?'resource.delete':'resource.restore',{resourceId},{type:deleted?'resource.restore':'resource.delete',resourceId},actor);return {changed:true};}
  ...
  decideProposal(id,status){const prop=this.db.prepare('SELECT resource_id FROM proposals WHERE id=?').get(id);this.db.prepare('UPDATE proposals SET status=?,decided_at=? WHERE id=?').run(status,now(),id);if(prop)this.refreshOrganizationState(prop.resource_id);}
  refreshOrganizationState(resourceId){const memberships=this.db.prepare('SELECT COUNT(*) c FROM memberships WHERE resource_id=?').get(resourceId).c, ready=this.db.prepare("SELECT COUNT(*) c FROM proposals WHERE resource_id=? AND status='ready'").get(resourceId).c;const state=ready?'suggestion_ready':memberships?'organized':'pending';this.db.prepare('UPDATE resources SET organization_state=?,updated_at=? WHERE id=?').run(state,now(),resourceId);}
  ...
  recoverInterruptedJobs(){const t=now();const res=this.db.prepare("UPDATE jobs SET status='paused',error=CASE WHEN error='' THEN 'TabAtlas restarted while this job was running; completed items are retained.' ELSE error END,updated_at=? WHERE status='running'").run(t);return Number(res.changes);}
  ...
  latestCapture(){const r=this.db.prepare('SELECT * FROM capture_runs ORDER BY started_at DESC LIMIT 1').get();return r?{...r,summary:p(r.summary_json)}:null;}
  stats(){const r=this.db.prepare("SELECT COUNT(*) total,SUM(archived_at IS NOT NULL) archived,SUM(archived_at IS NULL AND organization_state='pending') pending,SUM(archived_at IS NULL AND organization_state='suggestion_ready') suggestions,SUM(archived_at IS NULL AND organization_state='organized') organized,SUM(archived_at IS NULL AND evidence_state='limited') limited FROM resources WHERE deleted_at IS NULL").get();return Object.fromEntries(Object.entries(r).map(([k,v])=>[k,Number(v??0)]));}
}
