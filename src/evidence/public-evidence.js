...
export class PublicEvidenceService {
  ...
  async #youtube(resource,signal){
    ...
    try{const data=await this.#json(endpoint,signal);return {adapter:'youtube-oembed',kind:'metadata',status:'limited',sourceUrl:endpoint,payload:{title:data.title??resource.title,author:data.author_name??'',thumbnail:data.thumbnail_url??''},limitation:'Public oEmbed metadata available; transcript/captions were not fetched by this adapter.',contentHash:stableHash(data)};}catch(e){return limited(resource,'youtube-oembed',e.message);}
  }
  async #page(resource,signal){
    try{const text=await this.#text(resource.canonical_url,signal);const title=pick(text,/<title[^>]*>([\s\S]*?)<\/title>/i);const desc=pickMeta(text,'description');const image=pickMeta(text,'og:image');const social=resource.resource_type==='social';return {adapter:'public-html',kind:'metadata',status:text?'ok':'limited',sourceUrl:resource.canonical_url,payload:{title:decode(title)||resource.title,description:decode(desc),image,readableText:social?stripHtml(text).slice(0,4000):''},limitation:text?'':'No public HTML content returned.',contentHash:stableHash(text)};}catch(e){return limited(resource,'public-html',e.message);}
  }
