...
async function closeExact(command){const t=command.target;let observed={tabId:t.tabId,windowId:t.windowId,url:'',closed:false};try{const live=await chrome.tabs.get(t.tabId);observed.url=live.url||'';if(live.windowId!==t.windowId||canonical(live.url)!==canonical(t.url))throw new Error('Tab changed since it was saved');await chrome.tabs.remove(t.tabId);observed.closed=true;}catch(e){observed.error=e.message;}await req('/bridge/close-result',{method:'POST',body:JSON.stringify({actionId:command.id,observed})});}
function canonical(v){const u=new URL(v);u.hash='';u.hostname=u.hostname.toLowerCase();return u.toString();}
